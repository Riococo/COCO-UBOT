...
def multiman():
    ...
    if STRING_SESSION:
        try:
            bot.start()
            call_py.start()
            bot.loop.run_until_complete(ram_client(bot))
            user = bot.get_me()
    ...
    if STRING_2:
        try:
            MAN2.start()
            MAN2.loop.run_until_complete(ram_client(MAN2))
            user = MAN2.get_me()
            name = user.first_name
            uid = user.id
    ...
    if STRING_3:
        try:
            MAN3.start()
            MAN3.loop.run_until_complete(ram_client(MAN3))
            user = MAN3.get_me()
            name = user.first_name
            uid = user.id
    ...
    if STRING_4:
        try:
            MAN4.start()
            MAN4.loop.run_until_complete(ram_client(MAN4))
            user = MAN4.get_me()
            name = user.first_name
            uid = user.id
    ...
    if STRING_5:
        try:
            MAN5.start()
            MAN5.loop.run_until_complete(ram_client(MAN5))
            user = MAN5.get_me()
            name = user.first_name
            uid = user.id
